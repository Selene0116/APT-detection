...
def main(main_args):
    ...
    if dataset_name == 'streamspot' or dataset_name == 'wget':
        ...
        model.load_state_dict(
            torch.load("./checkpoints/checkpoint-{}.pt".format(dataset_name), map_location=device)
        )
        model = model.to(device)

        pooler = Pooling(main_args.pooling)
        test_auc, test_std = batch_level_evaluation(
            model, pooler, device, ['knn'], dataset_name, main_args.n_dim, main_args.e_dim
        )
        ...
        with torch.no_grad():
            ...
            # 4) 过滤不参与评估的 benign training samples
            test_idx = []
            ...
            # 5) KNN 评估
            test_auc, test_std, score, anomaly_idx = evaluate_entity_level_using_knn(
                dataset_name, x_train, result_x_test, result_y_test
            )

            print("\n===== Basic Output =====")
            print("Anomalous node indices (first 20):", anomaly_idx[:20])

            # 6) 按异常分数从高到低排序
            sorted_anomaly_idx = sorted(anomaly_idx, key=lambda i: score[i], reverse=True)

            # 7) 拆分 TP / FP
            tp_idx = [i for i in sorted_anomaly_idx if result_y_test[i] == 1.0]
            fp_idx = [i for i in sorted_anomaly_idx if result_y_test[i] == 0.0]

            print("\n===== Label Distribution Among Predicted Anomalous Nodes =====")
            print("label=1.0 count (TP candidates):", len(tp_idx))
            print("label=0.0 count (FP candidates):", len(fp_idx))

            # 8) 打印 TP 样例
            print("\n===== Top True Anomalous Nodes (TP Samples) =====")
            for i in tp_idx[:20]:
                print(
                    f"node_idx={i}, score={score[i]:.6f}, label={result_y_test[i]}"
                )

            # 9) 打印 FP 样例
            print("\n===== Top False Positive Nodes (FP Samples) =====")
            for i in fp_idx[:20]:
                print(
                    f"node_idx={i}, score={score[i]:.6f}, label={result_y_test[i]}"
                )

    print(f"\n#Test_AUC: {test_auc:.4f}±{test_std:.4f}")
    return
