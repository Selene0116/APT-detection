...
def read_single_graph(dataset, malicious, path, test=False):
    ...
    for l in f.readlines():
        ...
        # 按信息流/因果流方向规范化边方向。
        # READ/RECV/LOAD 表示对象影响进程，因此翻转为 object -> subject。
        if 'READ' in edge_type or 'RECV' in edge_type or 'LOAD' in edge_type:
            lines.append([dst, src, dst_type, src_type, edge_type, ts])
        else:
            lines.append([src, dst, src_type, dst_type, edge_type, ts])
    ...
    # 聚合同一有向节点对上的重复交互，保留行为频次、持续时间和边类型复杂度。
    edge_stats = {}
    ...
    for l in lines:
        ...
        if key not in edge_stats:
            edge_stats[key] = {
                ...
                'first_type': edge_type_id
            }

        edge_stats[key]['frequency'] += 1
        edge_stats[key]['first_ts'] = min(edge_stats[key]['first_ts'], ts)
        edge_stats[key]['last_ts'] = max(edge_stats[key]['last_ts'], ts)
        edge_stats[key]['type_counter'][edge_type_id] += 1

    for (u, v), stat in edge_stats.items():
        # 为了兼容 MAGIC 后续流程，仍然保留单一 type 字段。
        # 这里取出现次数最多的事件类型作为该节点对的代表交互类型。
        dominant_type = stat['type_counter'].most_common(1)[0][0]
        duration = stat['last_ts'] - stat['first_ts']

        g.add_edge(
            u,
            v,
            type=dominant_type,
            first_type=stat['first_type'],
            frequency=stat['frequency'],
            duration=duration,
            first_ts=stat['first_ts'],
            last_ts=stat['last_ts'],
            type_count=len(stat['type_counter'])
        )
