...
def build_model(args):
    ...
    mask_min_ratio = getattr(args, "mask_min_ratio", 0.3)
    mask_max_ratio = getattr(args, "mask_max_ratio", 1.5)
...
class GMAEModel(nn.Module):
    def __init__(self, n_dim, e_dim, hidden_dim, n_layers, n_heads, activation,
                 feat_drop, negative_slope, residual, norm, mask_rate=0.5, loss_fn="sce", alpha_l=2,mask_strategy="random", mask_min_ratio=0.3, mask_max_ratio=1.5):
        super(GMAEModel, self).__init__()
        self._mask_rate = mask_rate
        self._mask_strategy = mask_strategy
        self._mask_min_ratio = mask_min_ratio
        self._mask_max_ratio = mask_max_ratio
    ...
    def encoding_mask_noise(self, g, mask_rate=0.3):
        ...
        device = g.ndata["attr"].device

        if num_nodes == 0:
            empty = torch.empty(0, dtype=torch.long, device=device)
            return new_g, (empty, empty)

        if self._mask_strategy == "degree":
            ...
            mask_prob = mask_rate * (1.0 - deg_norm)
            mask_prob = mask_prob.clamp(
                min=mask_rate * self._mask_min_ratio,
                max=mask_rate * self._mask_max_ratio
            )

            mask_nodes = torch.nonzero(
                torch.rand(num_nodes, device=device) < mask_prob,
                as_tuple=False
            ).view(-1)

            # 防止极端情况下没有节点被 mask，保持训练目标有效。
            if mask_nodes.numel() == 0:
                perm = torch.randperm(num_nodes, device=device)
                mask_nodes = perm[: max(1, int(mask_rate * num_nodes))]

            mask_flag = torch.zeros(num_nodes, dtype=torch.bool, device=device)
            mask_flag[mask_nodes] = True
            keep_nodes = torch.nonzero(~mask_flag, as_tuple=False).view(-1)
        else:
            perm = torch.randperm(num_nodes, device=device)
