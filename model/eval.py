...
#             break
#     best_thres = threshold[best_idx]
#     pred = (score >= best_thres).astype(int)
#     anomaly_idx = np.where(pred == 1)[0]

#     print("Best threshold:", best_thres)
#     print("Number of predicted anomalous nodes:", len(anomaly_idx))
#     print("Predicted anomalous node indices:", anomaly_idx)
...
def evaluate_entity_level_using_knn(dataset, x_train, x_test, y_test):
    ...
    if not os.path.exists(save_dict_path):
        ...
        distances, _ = nbrs.kneighbors(
            x_train[idx][:min(50000, x_train.shape[0])],
    ...
    # precision_recall_curve 返回的 prec/rec 长度比 threshold 多 1，
    # 最后一个点没有对应阈值，因此只使用前 len(threshold) 个点选阈值。
    prec_t = prec[:-1]
    rec_t = rec[:-1]
    ...
    fp = 0
    for i in range(len(y_test)):
        if y_test[i] == 1.0 and pred[i] == 1:
            tp += 1
        if y_test[i] == 1.0 and pred[i] == 0:
            fn += 1
        if y_test[i] == 0.0 and pred[i] == 0:
            tn += 1
        if y_test[i] == 0.0 and pred[i] == 1:
            fp += 1

    print('AUC: {}'.format(auc))
    print('F1: {}'.format(f1_t[best_idx]))
    print('PRECISION: {}'.format(prec_t[best_idx]))
    print('RECALL: {}'.format(rec_t[best_idx]))
    print('Best threshold: {}'.format(best_thres))
    print('TN: {}'.format(tn))
    print('FN: {}'.format(fn))
    print('TP: {}'.format(tp))
    print('FP: {}'.format(fp))
    print('Predicted anomalous node count: {}'.format(len(anomaly_idx)))
    print('Predicted anomalous node indices (in result_x_test): {}'.format(anomaly_idx))

    return auc, 0.0, score, anomaly_idx