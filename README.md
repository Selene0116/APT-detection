...
如果需要查看某个数据集实际包含哪些节点类型和事件类型，可在原始 JSON 已放入 `data/<dataset>/` 后运行以下命令：
...
dataset = "cadets"  # 可改为 trace / theia / cadets
base = f"data/{dataset}"
...
for name in os.listdir(base):
    if "json" not in name or ".txt" in name or "names" in name or "types" in name:
        continue
    path = os.path.join(base, name)
    with open(path, "r", encoding="utf-8", errors="ignore") as f:
        for line in f:
            types = pattern_type.findall(line)
            if not types:
                if "com.bbn.tc.schema.avro.cdm18.MemoryObject" in line:
                    node_types["MemoryObject"] += 1
                elif "com.bbn.tc.schema.avro.cdm18.NetFlowObject" in line:
                    node_types["NetFlowObject"] += 1
                elif "com.bbn.tc.schema.avro.cdm18.UnnamedPipeObject" in line:
                    node_types["UnnamedPipeObject"] += 1
                continue
            ...
            if "com.bbn.tc.schema.avro.cdm18.Event" in line:
                ...
                "TimeMarker" not in line
                and "StartMarker" not in line
                and "EndMarker" not in line
                and "UnitDependency" not in line
                and "Host" not in line
            ):
                node_types[t] += 1

print("Node type features:")
for i, (name, count) in enumerate(sorted(node_types.items())):
    print(f"{i:02d}: {name} ({count})")

print("\nEdge/Event type features:")
for i, (name, count) in enumerate(sorted(edge_types.items())):
    print(f"{i:02d}: {name} ({count})")
PY
```
...
def entity_type_from_line(line):
    types = pattern_type.findall(line)
    if types:
        return types[0]
    if "com.bbn.tc.schema.avro.cdm18.MemoryObject" in line:
...
def build_feature_maps(dataset):
    base = os.path.join("data", dataset)
    id_nodetype_map = {}

    # First pass: collect UUID -> node type from entity records.
    for name in os.listdir(base):
        if (
            "json" not in name
            or ".txt" in name
        ...
        with open(os.path.join(base, name), "r", encoding="utf-8", errors="ignore") as f:
            for line in f:
                if "com.bbn.tc.schema.avro.cdm18.Event" in line:
                    continue
                if any(x in line for x in ["TimeMarker", "StartMarker", "EndMarker", "UnitDependency", "Host"]):
                    continue
                uuids = pattern_uuid.findall(line)
                if not uuids:
                    continue
                t = entity_type_from_line(line)
                if not t or uuids[0] == "00000000-0000-0000-0000-000000000000" or t == "SUBJECT_UNIT":
                    continue
                id_nodetype_map[uuids[0]] = t

    node_type_dict = OrderedDict()
    edge_type_dict = OrderedDict()

    def add_node_type(t):
        if t not in node_type_dict:
            node_type_dict[t] = len(node_type_dict)

    def add_edge_type(t):
        if t not in edge_type_dict:
            edge_type_dict[t] = len(edge_type_dict)

    # Second pass: follow trace_parser.py order and add only types used by graph edges.
    for split in ["train", "test"]:
        for name in metadata[dataset][split]:
            path = os.path.join(base, name)
            ...
            with open(path, "r", encoding="utf-8", errors="ignore") as f:
                for line in f:
                    if "com.bbn.tc.schema.avro.cdm18.Event" not in line:
                        continue
                    edge_types = pattern_type.findall(line)
                    src_ids = pattern_src.findall(line)
                    if not edge_types or not src_ids or src_ids[0] not in id_nodetype_map:
                        continue
                    src_type = id_nodetype_map[src_ids[0]]
                    for pat in [pattern_dst1, pattern_dst2]:
                        dst_ids = pat.findall(line)
                        if dst_ids and dst_ids[0] != "null" and dst_ids[0] in id_nodetype_map:
                            add_node_type(src_type)
                            add_node_type(id_nodetype_map[dst_ids[0]])
                            add_edge_type(edge_types[0])

    return node_type_dict, edge_type_dict

for dataset in datasets:
    node_map, edge_map = build_feature_maps(dataset)
    print(f"\n## {dataset.upper()}")
    print(f"node_feature_dim = {len(node_map)}")
    for name, idx in node_map.items():
        print(f"  node[{idx:02d}] = {name}")
    print(f"edge_feature_dim = {len(edge_map)}")
    for name, idx in edge_map.items():
        print(f"  edge[{idx:02d}] = {name}")
PY
```

脚本输出后，可将结果整理为如下格式写入实验报告或答辩 PPT：
...
> 本项目实现 APT 攻击检测与场景构建模块。输入是 DARPA TC 系统审计日志或预处理后的溯源图数据。系统首先构建进程、文件、网络连接等实体组成的异构有向溯源图，然后提取节点类型、入边事件分布和出边事件分布等行为语义特征。模型通过掩码图自编码学习正常系统行为的潜在表示分布，形成溯源图正常行为基线。测试阶段，系统根据节点相对正常行为基线的偏离程度计算异常分数，输出异常节点列表；随后以高置信异常节点为种子，结合时间约束、图约简和社区发现生成候选攻击场景图，实现从原始审计日志到攻击场景研判结果的完整流程。
